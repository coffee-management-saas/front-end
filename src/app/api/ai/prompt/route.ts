...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            console.error("[AI Prompt Proxy] Fetch error:", err);
            return Response.json(
                {
                    action: "INFO",
                    message: isTimeout
        ...
        try {
            ...
            // Nếu backend trả về field 'reply' thay vì 'message', map lại cho frontend
            if (data && typeof data === "object" && !("message" in data) && "reply" in data) {
                (data as any).message = (data as any).reply;
            }
            // Đảm bảo luôn có action if missing
            if (data && typeof data === "object" && !("action" in data)) {
                (data as any).action = "INFO";
            }
        } catch {
        ...
        console.error("[AI Prompt Proxy] Unexpected error:", err);
        return Response.json(
            {
                action: "INFO",
