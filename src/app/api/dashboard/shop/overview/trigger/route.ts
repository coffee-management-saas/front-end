...
export async function GET() {
  ...
  for (const base of bases) {
    const backendRes = await fetch(`${base}/dashboard/shop/overview/trigger`, {
      method: "GET",
      headers: {
    ...
    return Response.json(
      payload ?? { message: `Trigger dashboard failed (${backendRes.status})` },
  ...
  const message =
    ...
    typeof (lastPayload as Record<string, unknown>).message === "string"
      ...
      : `Trigger dashboard failed (${lastStatus || 500})`;
