...
export async function PUT(
  ...
  for (const base of bases) {
    const backendRes = await fetch(`${base}/notifications/${id}`, {
      method: "PUT",
      headers: {
    ...
    return Response.json(
      payload ?? { message: `Mark notification read failed (${backendRes.status})` },
  ...
  const message =
    ...
    typeof (lastPayload as Record<string, unknown>).message === "string"
      ...
      : `Mark notification read failed (${lastStatus || 500})`;
