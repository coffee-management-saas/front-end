...
export async function GET(req: Request) {
  ...
  const pageRaw = String(searchParams.get("page") ?? "0").trim();
  const sizeRaw = String(searchParams.get("size") ?? "100").trim();
  const page = Number(pageRaw);
  const size = Number(sizeRaw);

  const validInt = (n: number) => Number.isInteger(n) && n >= 0 && n <= 100_000;
  if (!validInt(page) || !validInt(size) || size < 1 || size > 1000) {
    return Response.json(
      { message: "Invalid paging (page/size)" },
      { status: 400 },
    );
  }
  ...
  for (const base of bases) {
    const backendRes = await fetch(`${base}/notifications?${qs.toString()}`, {
      method: "GET",
      headers: {
    ...
    return Response.json(
      payload ?? {
        message: `Load notifications failed (${backendRes.status})`,
      },
  ...
  const message =
    ...
    typeof (lastPayload as Record<string, unknown>).message === "string"
      ...
      : `Load notifications failed (${lastStatus || 500})`;
