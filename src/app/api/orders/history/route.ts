...
export async function GET(req: Request) {
  try {
    ...
    if (page == null || size == null || customerId == null) {
      return Response.json(
        { message: "Thiếu page/size/customerId" },
        { status: 400 },
      );
    }
    ...
    const beUrl = `${base}/orders/history?page=${encodeURIComponent(page)}&size=${encodeURIComponent(size)}&customerId=${encodeURIComponent(customerId)}`;
