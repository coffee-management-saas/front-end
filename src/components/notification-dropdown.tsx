...
export default function NotificationDropdown() {
    ...
    const currentNotifications = activeTab === "news" ? mockNewsNotifications : mockOrderNotifications;
            ...
            <div className="border-b border-gray-200">
                <div className="flex">
                    <button
                        ...
                        className={`flex-1 px-4 py-3 text-sm font-semibold transition-colors relative ${activeTab === "news"
                                ? "text-[#00796B]"
                    ...
                    <button
                        ...
                        className={`flex-1 px-4 py-3 text-sm font-semibold transition-colors relative ${activeTab === "orders"
                                ? "text-[#00796B]"
                                : "text-gray-500 hover:text-gray-700"
                            }`}
                    >
                ...
                {currentNotifications.length === 0 ? (
                    ...
                    <div className="divide-y divide-gray-100">
                        {currentNotifications.map((notification) => (
                            <div
                                key={notification.id}
                                className={`p-4 hover:bg-gray-50 transition-colors cursor-pointer ${!notification.read ? "bg-amber-50/30" : ""
                                    }`}
                            >
                                <div className="flex gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-2 mb-1">
                                            <h4 className={`text-sm font-semibold line-clamp-1 ${!notification.read ? "text-gray-900" : "text-gray-700"
                                                }`}>
                                                {notification.title}
                                            </h4>
                                            {!notification.read && (
